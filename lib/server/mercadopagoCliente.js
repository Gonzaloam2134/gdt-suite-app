...
// ---------------------------------------------------------------------------
// Fase B/C — detalle de un order (webhook) y búsquedas para conciliación
// ---------------------------------------------------------------------------

/** Detalle completo de un order — el webhook solo manda el id, hay que pedir el resto. */
export const obtenerOrder = (orderId, accessToken) => llamarMP(`/v1/orders/${orderId}`, { accessToken })

/** Orders de un tipo (qr | point) creados/actualizados en el rango — para la conciliación periódica. */
export const buscarOrders = ({ accessToken, type, beginDate, endDate }) => {
  const params = new URLSearchParams({
    begin_date: beginDate,
    end_date: endDate,
    ...(type ? { type } : {}),
  })
  return llamarMP(`/v1/orders?${params.toString()}`, { accessToken })
}

/** Pagos de la cuenta en el rango, opcionalmente filtrados por operation_type (ver esTransferencia en lib/domain). */
export const buscarPagos = ({ accessToken, operationType, beginDate, endDate, offset = 0, limit = 50 }) => {
  const params = new URLSearchParams({
    range: 'date_created',
    begin_date: beginDate,
    end_date: endDate,
  ...
  return llamarMP(`/v1/payments/search?${params.toString()}`, { accessToken })
}
