 ...
 * lista de locales que cubre cada una. Sin `!inner`: una cuenta sin ningún
 * local vivo igual tiene que aparecer, no desaparecer en silencio.
 */
export const listarSuscripcionesConOwner = (limite = 200) =>
  supabase.from('suscripciones_cuenta')
    .select('*, perfiles!suscripciones_cuenta_owner_id_fkey (email, nombre), locales!locales_creado_por_fkey (id, nombre, rubro)')
    .order('fecha_vencimiento', { ascending: true })
    .limit(limite)
    .then(unwrap)
    .then(cuentas => cuentas.map(c => ({
      ...c,
      ownerEmail: c.perfiles?.email || 'Sin email',
      cantidadLocales: c.locales?.length || 0,
    })))
