...
export default function CajaDelDia() {
  ...
  // NUEVO: Estado para expandir acreditaciones
    ...
    try {
      ...
      if (closedData && closedData.length > 0) {
        ...
        const { data: lastTx } = await supabase
          .from('transacciones')
          .select('monto, comision_monto, tipo, medio_pago_id')
  ...
  // ============================================
  // LÓGICA CONTABLE + ACREDITACIONES DEL DÍA
  // ============================================
  const hoy = new Date()
  ...
  const balanceByMethod = paymentMethods.map(method => {
    ...
    const commissions = methodMovements
      .filter(m => m.tipo === 'COBRO_RECIBIDO')
      .reduce((sum, m) => sum + (m.comision_monto || 0), 0)
    
    const expenses = methodMovements
      .filter(m => m.tipo === 'GASTO_REGISTRADO')
      .reduce((sum, m) => sum + m.monto, 0)
    
    const available = methodMovements
      .filter(m => {
    ...
    const inTransit = methodMovements
      .filter(m => {
        const isIncome = m.tipo === 'COBRO_RECIBIDO'
        const accreditationDate = m.fecha_acreditacion_estimada || hoyStr
        return isIncome && accreditationDate > hoyStr
    ...
    const netBalance = income - commissions - expenses
  ...
  // NUEVO: Calcular acreditaciones del día (ventas de días anteriores que acreditan HOY)
  const acreditacionesHoy = movements
    .filter(m => {
      ...
      // Solo ventas de días anteriores (no las de hoy, esas ya aparecen como cobros)
    ...
    .map(m => {
      ...
      const net = m.monto - (m.comision_monto || 0)
      return {
        ...m,
        method,
        net
  ...
  // Agrupar acreditaciones por medio de pago
  ...
  acreditacionesHoy.forEach(acc => {
    ...
    if (!acreditacionesAgrupadas[key]) {
      const methodName = acc.method ? 
        (acc.method.banco_emisor ? `${acc.method.nombre} (${acc.method.banco_emisor})` : acc.method.nombre) 
        : 'Medio desconocido'
      acreditacionesAgrupadas[key] = {
        method: acc.method,
        methodName,
        total: 0,
        transacciones: []
            ...
            <div style={{ backgroundColor: 'white', padding: '1rem', borderRadius: '10px', border: '1px solid #e2e8f0', marginBottom: '1rem' }}>
                ...
                <div style={{ backgroundColor: '#f0fdf4', padding: '0.75rem', borderRadius: '8px', border: '2px solid #16a34a' }}>
                  ...
                  {totalAcreditacionesHoy > 0 && (
                    <div style={{ fontSize: '0.65rem', color: '#16a34a', marginTop: '0.25rem' }}>
                      (+${totalAcreditacionesHoy.toFixed(2)} acreditan hoy)
                    </div>
                  )}
                </div>
              ...
              <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: '0.75rem', marginBottom: '0.75rem' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                  ...
                  <span style={{ fontSize: '0.875rem', fontWeight: '700', color: '#dc2626' }}>-${totalCommissions.toFixed(2)}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                  <span style={{ fontSize: '0.875rem', color: '#64748b' }}>Gastos:</span>
                  <span style={{ fontSize: '0.875rem', fontWeight: '700', color: '#dc2626' }}>-${totalExpenses.toFixed(2)}</span>
                </div>
                        ...
                        <div style={{ textAlign: 'right' }}>
                          ...
                          {inTransit > 0 && (
                            <div style={{ fontSize: '0.65rem', color: '#d97706' }}>
                              (${inTransit.toFixed(2)} en tránsito)
                            </div>
                          )}
                        </div>
                      </div>
                              ...
                              return (
                                <div key={t.id} style={{ 
                                  ...
                                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem' }}>
                                    <span style={{ color: '#64748b' }}>
                                      {t.descripcion} • ${t.monto.toFixed(2)} bruto
                                    </span>
                                    {t.comision_monto > 0 && (
                                      <span style={{ color: '#dc2626' }}>
                                        -${t.comision_monto.toFixed(2)} comisión
                                      </span>
                                    )}
                                  </div>
                                </div>
          ...
          <div style={{ backgroundColor: 'white', width: '100%', maxWidth: '500px', borderRadius: '12px', padding: '1.5rem', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ backgroundColor: '#f8fafc', padding: '1rem', borderRadius: '8px', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                ...
                <span style={{ fontWeight: '600', color: '#15803d' }}>${totalAvailable.toFixed(2)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                <span style={{ fontSize: '0.875rem' }}>En tránsito:</span>
                <span style={{ fontWeight: '600', color: '#d97706' }}>${totalInTransit.toFixed(2)}</span>
              </div>
              <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: '0.5rem', display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontWeight: '700', fontSize: '1.125rem' }}>${totalNet.toFixed(2)}</span>
              </div>
            </div>
