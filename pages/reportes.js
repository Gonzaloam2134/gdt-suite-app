  ...
  transacciones.forEach(t => {
    ...
    if (t.tipo === 'COBRO_RECIBIDO') {
      ...
      salesRows.push({
        ...
        medio: key,
        bruto: monto, neto, iva, comision, netoReal: neto - comision
      })
...
export default function Reportes() {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      expSales.forEach(row => ventasData.push([row.fecha, row.concepto, row.medio, row.bruto, row.neto, row.iva, row.comision, row.netoReal]))
      ...
      for (let col = 0; col < 8; col++) {
        applyStyle(wsVentas, `${['A','B','C','D','E','F','G','H'][col]}1`, { font: { bold: true, color: { rgb: 'FFFFFF' }, sz: 11 }, fill: { fgColor: { rgb: '1E293B' } }, alignment: { horizontal: 'center' } })
      }
      applyCurrencyFormat(wsVentas, 2, ventasData.length, 3, 7)
      applyDateFormat(wsVentas, 2, ventasData.length - 1, 0)
      
      const totalesRowVentas = ventasData.length
      for (let col = 0; col < 8; col++) {
        applyStyle(wsVentas, `${['A','B','C','D','E','F','G','H'][col]}${totalesRowVentas}`, { font: { bold: true, sz: 11 }, fill: { fgColor: { rgb: 'F1F5F9' } }, border: { top: { style: 'double', color: { rgb: '0F172A' } } } })
      }
