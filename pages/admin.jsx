...
export default function AdminPanel() {
  ...
  const { local, stats, miembros, mediosPago, logs, periodo, loading, aplicarPreset, aplicarFechas, recargar } = useAdminData()
  ...
  if (!activeLocalId) {
    return (
      <main className="min-h-screen bg-slate-100">
        <AdminHeader titulo="Administración" />
        <div className="max-w-6xl mx-auto p-4">
          <p className="text-sm text-gray-600">Elegí un local desde Mis Locales para administrarlo.</p>
        </div>
  ...
  if (!esOwner) {
    return (
      <main className="min-h-screen bg-slate-100 pb-20">
        <AdminHeader titulo="Mi actividad" subtitulo={local?.nombre} />
  ...
  return (
    <main className="min-h-screen bg-slate-100 pb-20">
      <AdminHeader titulo="Administración" subtitulo={local?.nombre} />
      ...
      <div className="max-w-6xl mx-auto p-4">
        ...
        {tab === 'miembros' && (
          <MiembrosTab miembros={miembros} localId={activeLocalId} userId={userId} onCambio={recargar} />
        )}
        {tab === 'medios-pago' && (
